...
function on_reactor_9108000(reactor, item)
	if item ~= nil then
		return
	end
	...
	if player == nil then
		log("on_reactor_9108000: trigger is nil")
		return
	end
	local sm = player:state_machine()
	if sm == nil then
		log("on_reactor_9108000: state_machine is nil for char", player:id())
		return
	end
	local group = sm:group()
	if group == nil then
		log("on_reactor_9108000: sm group is nil")
		return
	end
	if group:name() ~= GROUP_NAME then
		log("on_reactor_9108000: unexpected group", group:name(), "expected", GROUP_NAME)
		return
	end
