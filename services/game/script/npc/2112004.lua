...
local GROUP_NAME = "romeo_party_quest"
local ENTRY_MAP = 261000011
...
local function try_start(me, npc)
	...
	if sm == nil then
		me:dialog(npc, "파티 퀘스트를 시작할 수 없습니다.")
		if err ~= nil then
			log("romeo_party_quest start_party:", err)
		end
	end
end
...
local function handle_hub(me, npc, sm, map)
	...
	if pq.has_item(me, 4001135, 1) then
		pq.remove_all(4001135, me)
		rj.clear_fx(map)
		map:block_gen(true)
		map:kill_all_mobs()
		pq.party_exp(sm, 5000)
		sm:set_property("stage4", "2")
		local door = map:reactor_by_name("rnj3_out3")
		if door ~= nil then
			door:hit(1)
		end
...
local function handle_reward(me, npc, marble, exit_map)
	me:dialog(npc, "이렇게 유레테도 마음을 고쳐먹었고, 제뉴미스트와 알카드노의 전쟁도 조금은 수그러 들 수 있을까요? 비록 저희 사랑이 이뤄지기까지 아직도 험난한 고난이 기다리고 있지만, 결코 포기하지 않고 줄리엣을 지키겠습니다.")
	local code = me:exchange({}, { item = { [marble] = 1 } })
...
return {
	on_click = function(me, npc)
		...
		if map_id == 926100000 then
			me:dialog(npc, "이 연구실에서 가끔 수상한 소리가 들린다는 소문이 있었어요. 분명 이 근처 어딘가에 수상한 소리의 근원이 있을겁니다.")
		elseif map_id == 926100001 then
			me:dialog(npc, "모든 몬스터를 물리쳐 주십시오.")
		elseif map_id == 926100100 then
			me:dialog(npc, "이곳의 비커들은 깨져서 내용물이 새고 있습니다. 비커에 내용물이 모두 새기 전에 빠르게 수상한 액체를 채워 넣어주십시오.")
		elseif map_id == 926100200 then
			if sm == nil then
				return
			end
			handle_hub(me, npc, sm, map)
		elseif map_id == 926100300 then
			me:dialog(npc, "각 파티원이 각각 연구실을 통해 꼭대기에 있는 중앙 연구실 까지 올라가야 해요.")
		elseif map_id == 926100400 then
			me:dialog(npc, "준비가 되면, 제 사랑을 구하러 가요.")
		elseif map_id == 926100401 then
			if sm ~= nil then
				pq.party_warp(sm, 926100500)
			end
		elseif map_id == 926110401 then
			me:dialog(npc, "구해주셔서 진심으로 감사드립니다.")
		elseif map_id == 926100600 then
			handle_reward(me, npc, 4001159, 926100700)
