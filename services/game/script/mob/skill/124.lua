-- Mob skill name (Skill.wz/MobSkill.img.xml): mob skill 124

function on_mob_skill_choose_124(mob, skill)
    return true
end

function on_mob_skill_124(mob, controller, skill)
    ...
    for _, ch in ipairs(targets) do
        ch:debuff(DebuffFlag.Curse, time, x, skill:id(), skill:level())
    end
    return true
end
