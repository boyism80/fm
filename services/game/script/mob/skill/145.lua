-- Mob skill name (Skill.wz/MobSkill.img.xml): mob skill 145
...
return {
	...
	on_mob_skill = function(mob, controller, skill)
		...
		for _, target in ipairs(targets) do
		    target:buff(MobBuff.WeaponDamageReflect, effect.x, effect.time, skill, controller)
		    target:buff(MobBuff.WeaponImmunity, effect.x, effect.time, skill, controller)
		    target:buff(MobBuff.MagicDamageReflect, effect.x, effect.time, skill, controller)
		    target:buff(MobBuff.MagicImmunity, effect.x, effect.time, skill, controller)
		end
		return true
	end
}
