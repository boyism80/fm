-- Mob skill name (Skill.wz/MobSkill.img.xml): mob skill 120

function on_mob_skill_choose_120(mob, skill)
    return true
end

function on_mob_skill_120(mob, controller, skill)
    ...
    for _, ch in ipairs(targets) do
        ch:debuff(DebuffFlag.Seal, time, x, skill:id(), skill:level())
    end
    return true
end
