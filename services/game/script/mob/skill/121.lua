-- Mob skill name (Skill.wz/MobSkill.img.xml): mob skill 121

function on_mob_skill_choose_121(mob, skill)
    return true
end

function on_mob_skill_121(mob, controller, skill)
    ...
    for _, ch in ipairs(targets) do
        ch:debuff(DebuffFlag.Darkness, time, x, skill:id(), skill:level())
    end
    return true
end
