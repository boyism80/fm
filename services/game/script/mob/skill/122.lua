-- Mob skill name (Skill.wz/MobSkill.img.xml): mob skill 122

function on_mob_skill_choose_122(mob, skill)
    return true
end

function on_mob_skill_122(mob, controller, skill)
    ...
    for _, ch in ipairs(targets) do
        ch:debuff(DebuffFlag.Weaken, time, x, skill:id(), skill:level())
    end
    return true
end
