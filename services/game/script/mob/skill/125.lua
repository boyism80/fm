-- Mob skill name (Skill.wz/MobSkill.img.xml): mob skill 125

function on_mob_skill_choose_125(mob, skill)
    return true
end

function on_mob_skill_125(mob, controller, skill)
    ...
    for _, ch in ipairs(targets) do
        ch:debuff(DebuffFlag.Poison, time, x, skill:id(), skill:level())
        ch:add_hp(-x)
    end
    return true
end
