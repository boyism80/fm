-- Mob skill name (Skill.wz/MobSkill.img.xml): mob skill 126

function on_mob_skill_choose_126(mob, skill)
    return true
end

function on_mob_skill_126(mob, controller, skill)
    ...
    for _, ch in ipairs(targets) do
        ch:debuff(DebuffFlag.Slow, time, x, skill:id(), skill:level())
    end
    return true
end
