-- Mob skill name (Skill.wz/MobSkill.img.xml): mob skill 123

function on_mob_skill_choose_123(mob, skill)
    return true
end

function on_mob_skill_123(mob, controller, skill)
    ...
    for _, ch in ipairs(targets) do
        ch:debuff(DebuffFlag.Stun, time, x, skill:id(), skill:level())
    end
    return true
end
