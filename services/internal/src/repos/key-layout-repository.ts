...
export class KeyLayoutRepository extends ValueRepository<KeyLayoutModel, KeyLayoutRow, number> {
    ...
    onUpsert(row: KeyLayoutRow): RepositoryQuery {
        return {
    ...
    onBulkUpsert(rows: KeyLayoutRow[]): RepositoryQuery | null {
        if (!rows.length) {
            return null;
        }
    ...
    onDelete(row: KeyLayoutDeleteModel): RepositoryQuery {
        return {
    ...
    rowToModel(row: KeyLayoutRow): KeyLayoutModel {
        return {
    ...
    modelToRow(model: KeyLayoutModel): KeyLayoutRow {
        return {
