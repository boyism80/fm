...
`);
        ...
        await sequelize.query("ALTER TABLE character_name_registry DROP COLUMN IF EXISTS deleted_at");
        await sequelize.query("CREATE UNIQUE INDEX idx_character_name_lower ON character_name_registry (LOWER(name))");
    },

    async down(queryInterface) {
        const { sequelize } = queryInterface;
        await sequelize.query("DROP INDEX IF EXISTS idx_character_name_lower");
        await sequelize.query("ALTER TABLE character_name_registry ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP");
        await sequelize.query(`
