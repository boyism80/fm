...
function on_attack_3221005(me, skill, damages)
	...
	for mob, hits in pairs(damages) do
		if mob ~= nil and summon_attack_hits_positive(hits) then
			if math.random(1, 100) <= prop then
				mob:set_status(MobStatus.Freeze, 1, duration_ms, skill, me)
			end
		end
	end
end

function on_activated_3221005(me, skill, params)
...
function on_buff_3221005(me, skill)
	if me == nil or skill == nil then
		return
	end
	local wz = skill:wz()
...
function on_unbuff_3221005(me, skill)
	if me == nil or skill == nil then
		return
	end
	local wz = skill:wz()
